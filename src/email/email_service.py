...
from ..models import Client, RiskAlert, EmailReport, SectorType, RiskLevel
...
class EmailService:
    ...
    def generate_daily_report(self, client: Client, alerts: List[RiskAlert]) -> Dict[str, str]:
        ...
        stats = {
            ...
            'top_sector': max(alerts_by_sector.keys(), key=lambda x: len(alerts_by_sector[x])) if alerts_by_sector else None
        }
    ...
    def generate_critical_alert(self, client: Client, alert: RiskAlert) -> Dict[str, str]:
...
"""
    ...
    async def get_client_alerts(self, client: Client, hours_back: int = 24) -> List[RiskAlert]:
        ...
        async with AsyncSessionLocal() as db:
            ...
            if client.states:
                query = query.join(RiskAlert.article).join(Article.source).where(
                    or_(*[Source.state.ilike(f"%{state}%") for state in client.states])
                )
        ...
        async with AsyncSessionLocal() as db:
            ...
            for client in clients:
                try:
                    ...
                    logger.error(f"Error sending daily report to {client.email}: {str(e)}")
                    results["failed"] += 1
            ...
            for alert in critical_alerts:
                ...
                for client in relevant_clients:
                    try:
                        ...
                        logger.error(f"Error sending critical alert to {client.email}: {str(e)}")
                        results["failed"] += 1
        ...
        async with AsyncSessionLocal() as db:
            ...
            for client in clients:
                try:
                    ...
                    alerts = await self.get_client_alerts(client, hours_back=168)  # 7 days
                    ...
                    logger.error(f"Error sending weekly summary to {client.email}: {str(e)}")
                    results["failed"] += 1
        
        return results
    
    def generate_weekly_summary(self, client: Client, alerts: List[RiskAlert]) -> Dict[str, str]:
